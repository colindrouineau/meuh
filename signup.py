...
def signup():
    ...
    if form.submit.data:
        ...
        with app.app_context():
            ...
            create_user(
                d["id"], d["first_name"], d["last_name"], d["password"], d["email"]
            )
        ...
        with app.app_context():
            ...
            if d["id"] in [user.username for user in users]:
                ...
                if user.password == d["password"]:
                    d["first_name"] = (
                        User.query.filter_by(username=d["id"]).first().first_name
                    )
                    d["last_name"] = (
                        User.query.filter_by(username=d["id"]).first().last_name
                    )
                    return redirect(url_for("accueil"))
                else:
                    return '<p>mot de passe incorrect</p> <hr> <a href="/login"> Réessayez !</a> <hr> <a href="/signup">Inscrivez-vous !</a>'
            else:
